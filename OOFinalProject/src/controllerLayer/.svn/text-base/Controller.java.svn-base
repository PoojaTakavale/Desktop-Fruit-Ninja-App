...
import modelLayer.Fruits;
import modelLayer.User;
import modelLayer.UserDB;
import factory.Bomb;
import factory.Factory;
import factory.Fruit;
import strategy.Level1;
import strategy.Level2;
import strategy.Level3;
import strategy.Strategy;
import viewLayer.GamePanel;

// controller layer, implement runnable
// first get username, if user a username not in database 
// and not an empty string, the game start
// fruit appears in screen and user move mouse to slice fruit
// to earn points. If user miss one fruit or touch bomb
// game over and the score will store under this user's account
// and user can go to a public html to see their score
// https://s3-us-west-2.amazonaws.com/oofinalproject/oofinalprojectobject
...
public class Controller implements Runnable {
	...
	public static final int FREQUENCY = 2; // generate a fruit for every 2 seconds
...
// set start in controller, if get name not in database, return true
	public boolean verifyUser(){
		if( this.user.getUsername().length() == 0 ) { return false; }
		...
		if( sameName.size() == 0 ){ return true; }
		String selectExpression = "Select * from `oofinalproject` where username = '" + this.user.getUsername() + "' and password = '" + this.user.getPassword() + "'";
...
//			automatically change to higher level base on how long the user play
...
//	show all fruits in view
	public void paint( Graphics g){
		...
		for( int i = 0; i < size; i++ ){
			...
			g.drawImage(image, x - model.getFruits().get(i).getRadius(), y - model.getFruits().get(i).getRadius(), model.getFruits().get(i).getRadius(), model.getFruits().get(i).getRadius(), null);
		}
	}
...
//	if hit fruit, earn points else if hit bomb, set dead = true
	public void hitFruit(){
		...
		int distance = 0;
		for( int i = 0; i < size; i++ ){
			distance = (int)Math.pow( Math.abs(mouse.getCenter().x - model.getFruits().get(i).getCenter().x), 2 ) + (int)Math.pow( Math.abs(mouse.getCenter().y - model.getFruits().get(i).getCenter().y), 2);
...
//	check if fruit is out of view, then game over
	public void checkLost() {
		int size = model.getFruits().size();
		for (int i = 0; i < size; i++) {
			if (model.getFruits().get(i).getCenter().getX() < 0
					|| model.getFruits().get(i).getCenter().getY() < 0
