...
// save user information in Amazon SimpleDB and Amazon S3
// data save in SimpleDB with domain name "oofinalproject" 
// and in S3 save in bucket with bucketname "oofinalproject"
// S3 bucket only has one object "oofinalprojectobject"
// update s3 object named "oofinalprojectobject" when add new user
package modelLayer;
...
public class UserDB {
	...
	public static AmazonSimpleDB loginSimpleDB(String accessKey,
			String secretKey) {
	...
	public void addUser(User user) {
		...
		ReplaceableItem userItem = new ReplaceableItem(user.getUsername())
				.withAttributes(
						new ReplaceableAttribute("username",
								user.getUsername(), true),
						new ReplaceableAttribute("date", user.getDate(), true),
						new ReplaceableAttribute("score", Integer.toString(user
								.getScore() + maskForInteger), true ),
						new ReplaceableAttribute( "password", user.getPassword(), true));

		data.add(userItem);
		db.batchPutAttributes(new BatchPutAttributesRequest( domainName,
				data));
//		SimpleDB sometime may has latency issue and if write to Amazon S3 immediately 
...
//		System.out.println( "search by" );
	...
	private File createSampleFile() throws IOException {
		...
		for( int i = 0; i < size; i++ ){
			...
			writer.write("<td>" + Integer.toString(Integer.parseInt( items.get(i).getAttributes().get(1).getValue()) - maskForInteger) + "</td>\n");
